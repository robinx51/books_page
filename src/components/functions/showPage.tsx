...
export function resultPage(books: BookData[] | null) {
  const { addToFavorites, isBookInFavorites, removeFromFavorites } = useFavorites();
          ...
          <>
            {booksToDisplay.map((book) => (
              <div className="book secondColor" key={book.id}>
                ...
                <div className="info">
                  <h3 title={book.title} className='three-line-ellipsis'>{book.title}</h3>
                  <p title={book.authors.join(', ')} className='three-line-ellipsis'>Автор(ы): {book.authors.join(', ')}</p>
                  {isBookInFavorites(book) ? <button onClick = {() => removeFromFavorites(book)}>Удалить из избранных</button> :
                    <button onClick={() => addToFavorites(book)}>Добавить в избранное</button>}
                </div>
              </div>
            ))}
          </>
...
export function navigationBar() {
  ...
  return(
      <div className="navigationBar mainColor">
        ...
        <h2 className='favoritesButton'>
          {currentPathname == "/favorites" ? <>Избранное</> :
